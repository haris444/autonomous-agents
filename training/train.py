...
def train(config: Config = None, visualize: bool = False,
    ...
    if visualize:
        import os
        from analysis.visualize import GridRenderer, TrainingLogger, EpisodeRecorder
    ...
    for update in range(num_updates):
        ...
        if num_teams is not None and num_teams > 1:
            from training.scenarios import MultiTeamScenario
...
if __name__ == "__main__":
    ...
    if args.enemy_prob is not None:
        from training.scenarios import CURRICULUM
        if 9 in CURRICULUM:
