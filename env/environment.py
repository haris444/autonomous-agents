...
class GridWorld:
    ...
    def reset(self) -> Dict[str, torch.Tensor]:
        ...
        self.predator_kills_total = torch.zeros(self.n_agents, device=self.device)
    ...
    def _resolve_attacks(self, directions: torch.Tensor, attack_mask: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if hit_indices.numel() > 0:
            ...
            # Lifesteal: attacker heals fraction of damage dealt
            if self.config.lifesteal_fraction > 0:
                lifesteal = damage_matrix.sum(dim=1) * self.config.lifesteal_fraction
                self.agent_hp = torch.min(self.agent_hp + lifesteal,
                                          torch.full_like(self.agent_hp, self.config.max_hp))
            ...
            if pred_hit_indices.numel() > 0:
                ...
                for i in range(pred_hit_indices.numel()):
                    ...
                    if self.predator_alive[p_idx]:
                        ...
                        # Lifesteal from predator damage
                        if self.config.lifesteal_fraction > 0:
                            heal = pred_damage[i] * self.config.lifesteal_fraction
                            self.agent_hp[pred_hit_indices[i]] = torch.min(
                                self.agent_hp[pred_hit_indices[i]] + heal,
                                torch.tensor(self.config.max_hp, device=self.device))
                        ...
                        if self.predator_hp[p_idx] <= 0:
                            ...
                            self.predator_kills_total[pred_hit_indices[i]] += 1
    ...
    def _resolve_give_food(self, directions: torch.Tensor, give_mask: torch.Tensor) -> torch.Tensor:
        ...
        has_target = (target_ids >= 0) & (target_ids < self.n_agents)
        valid_target_ids = target_ids.clamp(min=0, max=self.n_agents - 1)
    ...
    def _compute_hierarchy_rewards(self) -> torch.Tensor:
        ...
        # Predator kills
        kills = self.predator_kills_total * alive
        kills_max = kills.max().clamp(min=eps)
        kills_norm = kills / kills_max

        # Weighted hierarchy score
        score = (cfg.hierarchy_food_weight * food_norm
                 + cfg.hierarchy_damage_weight * damage_norm
                 + cfg.hierarchy_hp_weight * hp_norm
                 + cfg.hierarchy_kill_weight * kills_norm) * alive
    ...
    def _check_curriculum_advance(self, episode_return: float) -> None:
        ...
        from training.scenarios import THRESHOLDS
